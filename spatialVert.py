# This file named 'spatialVert' provides the classes that create the 2D map that
...
# This model specifically focuses on current steerig in between layers,
# due to differences in data for the accuracy of the model minor things such
# as all firing rates being halved have been included

# Created by Sam Murray and assisted by Proffessor Yan Wong
# Created on the 15/01/2024
...
class Grid:
    ...
    def create_grid(self) -> list[Tile]:
        ...
        for y in range(self.height):
            ...
            perm = 1.2*(10**(-7)) # This is a standard value for white matter
    ...
    def calc_Ifield(self) -> None:
        ...
        for tile in self.tiles:
            ...
            for coord, amp in self.current_sources.items():
                ...
                partial_I = amp * (np.exp(-0.5 * (d * ap1 / temp_cond) ** 2) + 0.7  * np.exp(-0.5 * (d * ap2 / temp_cond) ** 2) - 0.3 * np.exp(-0.5 * (d * ap3 / temp_cond) ** 2)) / 35700
    ...
    def establish_connectivities(self, row1: int, row2: int, weight: float, prob: float) -> None:
        ...
        # print(f"Establishing connection of prob {prob} from {row1} to {row2}")
    ...
    def simulate(self, duration) -> tuple[list[float]]: 
        ...
        # We are dividing by 2 here cause vertical firing rate is lower and the model
        # isnt sensitive enough to change in that range
        spike_array = np.reshape(spike_data, (self.height,self.width))/2
        spike_array_pn = np.reshape(spike_data_pn, (self.height,self.width))/2
